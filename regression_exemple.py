...
# Interpolation defaulting
@Craft()
def spam(masterFile: Artefact, samples=10) -> Volatile("foo"):
    return 1
...
@Craft()
def spam(masterFile: Artefact, samples=11) -> Volatile("foo"):
    return 1
...
@Craft()
def spam(masterFile: Artefact = default, samples=11) -> Volatile("foo"):
    return 1
...
@Craft()
def foo(masterFile: Artefact, samples=10, YEAR=10) -> Volatile("df"):
    print(f"foo got {YEAR}")
    return masterFile


@Craft()
def spam(df: Volatile, YEAR=5):
    print(f"foo got {YEAR}")
    print(df)
...
with sess:
    namespaced = {"foo": {"YEAR": 1}}

    out = p(namespaced=namespaced, YEAR=8)
...
# @_Craft.hook_on_error(last=False)
